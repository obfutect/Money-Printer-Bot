...
2. `--config <path>`, `-c <path>` або `--config=<path>`
...
Опціональна секція вбудованої веб-панелі.

Ця секція керує вбудованим браузерним інтерфейсом і портом, на якому він слухає, але параметри командного рядка все одно можуть перевизначати частину значень під час старту.

### `server_enabled`

Type: `bool`, optional

Увімкнює вбудований серверний режим із конфігурації.

Поточна поведінка:
- якщо значення `true`, бот запускає вбудовану веб-панель без необхідності передавати `--server`
- якщо значення `false` або поле відсутнє, серверний режим усе одно можна ввімкнути з командного рядка через `--server`

### `password`

Type: `string`, optional

Пароль для браузерної панелі.

Це startup-fixed поле і воно має бути задане буквальним рядком.

Фактичний пріоритет пароля під час старту:
1. `BOT_PASSWD`
2. `--server-pass`
3. `server_config.password`
4. випадковий пароль, згенерований і виведений під час запуску

### `port`

Type: `u16`, optional

Порт прослуховування вбудованого сервера.

Поточна поведінка:
- значення за замовчуванням — `9090`
- `--port` перевизначає `server_config.port`

## `[auto_unwrap]`

Опціональна секція захисту балансу WSOL.

Ця секція має сенс, коли `base_token` дорівнює WSOL.

### `enabled`

Type: bindable `bool`

Визначає, чи дозволено боту автоматично розгортати WSOL, коли баланс SOL у гаманці падає нижче заданого мінімуму.

### `minimum_balance`

Type: bindable `u64`, optional

Мінімальний баланс SOL у гаманці в сирих лампортах.

Поточна поведінка:
- якщо це поле відсутнє, обмеження на відправлення за балансом не застосовується
- якщо поле присутнє і `enabled = false`, звичайне відправлення пропускається, доки баланс SOL нижчий за поріг
- якщо поле присутнє і `enabled = true`, бот може відправити сервісну транзакцію, щоб відновити SOL із WSOL

Поточна реалізація auto-unwrap:
- сервісний шлях є незалежним від звичайного торгового відправлення
- будується одна versioned transaction, яка закриває канонічний WSOL ATA і заново створює його в тій самій транзакції
- повторні спроби тривають, доки транзакцію не буде прийнято і не буде підтверджено післяумову
- якщо доступного WSOL усе одно недостатньо, щоб підняти баланс SOL вище заданого мінімуму, звичайне відправлення залишається пропущеним, доки баланс нижчий за поріг
...
### 7. Пауза впливає на торгівлю, але не на всі сервісні задачі

Пауза бота зупиняє звичайне торгове відправлення, але незалежні сервісні задачі за потреби можуть продовжувати працювати.

### 8. Ручний режим market не зупиняє оновлення LUT

Ручний режим market пригнічує автоматичне перезавантаження `markets` із налаштованого джерела, але перезавантаження LUT продовжує працювати.
