...
2. `--config <path>`, `-c <path>` או `--config=<path>`
...
תצורה אופציונלית עבור לוח הבקרה המובנה בדפדפן.

הסקציה הזו שולטת בממשק הדפדפן המובנה ובפורט שעליו הוא מאזין, אבל דגלי שורת הפקודה עדיין יכולים לדרוס חלק מהערכים בעת האתחול.

### `server_enabled`

Type: `bool`, optional

מפעיל את מצב השרת המובנה מתוך התצורה.

התנהגות נוכחית:
- אם הערך הוא `true`, הבוט מפעיל את לוח הבקרה המובנה בלי צורך ב-`--server`
- אם הערך הוא `false` או לא מוגדר, עדיין אפשר להפעיל את מצב השרת משורת הפקודה באמצעות `--server`

### `password`

Type: `string`, optional

הסיסמה ללוח הבקרה בדפדפן.

זהו שדה startup-fixed והוא חייב להיות מחרוזת ליטרלית.

סדר העדיפות בפועל של הסיסמה בעת האתחול:
1. `BOT_PASSWD`
2. `--server-pass`
3. `server_config.password`
4. סיסמה אקראית שנוצרת ומודפסת בעת האתחול

### `port`

Type: `u16`, optional

פורט ההאזנה של השרת המובנה.

התנהגות נוכחית:
- ברירת המחדל היא `9090`
- `--port` דורס את `server_config.port`

## `[auto_unwrap]`

סקציה אופציונלית להגנת יתרת WSOL.

הסקציה הזו רלוונטית כאשר `base_token` הוא WSOL.

### `enabled`

Type: bindable `bool`

קובע האם הבוט רשאי לבצע unwrap אוטומטי ל-WSOL כאשר יתרת ה-SOL בארנק יורדת מתחת למינימום שהוגדר.

### `minimum_balance`

Type: bindable `u64`, optional

יתרת המינימום של SOL בארנק, בלמפורטים גולמיים.

התנהגות נוכחית:
- אם השדה הזה לא מוגדר, לא מוחל שער חסימת שליחה המבוסס על יתרה
- אם השדה מוגדר ו-`enabled = false`, שליחה רגילה תדולג כל עוד יתרת ה-SOL נמוכה מהסף
- אם השדה מוגדר ו-`enabled = true`, הבוט עשוי לשלוח טרנזקציית תחזוקה כדי לשחזר SOL מתוך WSOL

המימוש הנוכחי של auto-unwrap:
- מסלול התחזוקה עצמאי משליחת הטריידים הרגילה
- נבנית טרנזקציה versioned אחת שסוגרת את ה-WSOL ATA הקנוני ויוצרת אותו מחדש באותה טרנזקציה
- הניסיונות ממשיכים עד שהטרנזקציה מתקבלת ומצב היעד מאומת
- אם כמות ה-WSOL הזמינה עדיין לא מספיקה כדי להעלות את יתרת ה-SOL מעל המינימום שהוגדר, השליחה הרגילה תישאר מדולגת כל עוד היתרה נמוכה מהסף
...
### 7. Pause משפיע על מסחר, לא על כל התחזוקה

השהיית הבוט עוצרת שליחת טריידים רגילה, אבל משימות תחזוקה עצמאיות עדיין יכולות להמשיך כשצריך.

### 8. מצב שוק ידני לא עוצר עדכוני LUT

מצב שוק ידני מדכא טעינה מחדש אוטומטית של `markets` מהמקור המוגדר, אבל טעינות מחדש של LUT עדיין נמשכות.
