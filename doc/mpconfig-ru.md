...
2. `--config <path>`, `-c <path>` или `--config=<path>`
...
При первом запуске файл keypair шифруется автоматически, если он еще не зашифрован.

Это startup-fixed значение и оно не может быть задано через плейсхолдер.

## `[server_config]`

Опциональная секция встроенной веб-панели.

Эта секция управляет встроенным браузерным интерфейсом и портом, на котором он слушает, но параметры командной строки все равно могут переопределять часть значений при старте.

### `server_enabled`

Type: `bool`, optional

Включает встроенный серверный режим из конфигурации.

Текущее поведение:
- если значение `true`, бот запускает встроенную веб-панель без необходимости передавать `--server`
- если значение `false` или поле отсутствует, серверный режим все равно можно включить из командной строки через `--server`

### `password`

Type: `string`, optional

Пароль для браузерной панели.

Это startup-fixed поле и оно должно быть задано литеральной строкой.

Фактический приоритет пароля при старте:
1. `BOT_PASSWD`
2. `--server-pass`
3. `server_config.password`
4. случайный пароль, сгенерированный и выведенный при запуске

### `port`

Type: `u16`, optional

Порт прослушивания встроенного сервера.

Текущее поведение:
- по умолчанию используется `9090`
- `--port` переопределяет `server_config.port`

## `[auto_unwrap]`

Опциональная секция защиты баланса WSOL.

Эта секция имеет смысл, когда `base_token` равен WSOL.

### `enabled`

Type: bindable `bool`

Определяет, разрешено ли боту автоматически разворачивать WSOL, когда баланс SOL в кошельке падает ниже заданного минимума.

### `minimum_balance`

Type: bindable `u64`, optional

Минимальный баланс SOL в кошельке в сырых лампортах.

Текущее поведение:
- если это поле отсутствует, ограничение на отправку по балансу не применяется
- если поле присутствует и `enabled = false`, обычная отправка пропускается, пока баланс SOL ниже порога
- если поле присутствует и `enabled = true`, бот может отправить сервисную транзакцию, чтобы восстановить SOL из WSOL

Текущая реализация auto-unwrap:
- сервисный путь независим от обычной торговой отправки
- строится одна versioned transaction, которая закрывает канонический WSOL ATA и заново создает его в той же транзакции
- повторные попытки продолжаются, пока транзакция не будет принята и постусловие не будет подтверждено
- если доступного WSOL все равно недостаточно, чтобы поднять баланс SOL выше заданного минимума, обычная отправка остается пропущенной, пока баланс ниже порога
...
### 7. Пауза влияет на торговлю, но не на все сервисные задачи

Пауза бота останавливает обычную торговую отправку, но независимые сервисные задачи при необходимости могут продолжать работу.

### 8. Ручной режим market не отключает обновления LUT

Ручной режим market отключает автоматическую перезагрузку `markets` из настроенного источника, но перезагрузка LUT продолжает работать.
