...
2. `--config <path>` أو `-c <path>` أو `--config=<path>`
...
إعداد اختياري للوحة الويب المدمجة.

يتحكم هذا القسم في واجهة المتصفح المدمجة والمنفذ الذي تستمع عليه، لكن معاملات سطر الأوامر لا تزال قادرة على تجاوز بعض قيمه عند بدء التشغيل.

### `server_enabled`

Type: `bool`, optional

يُفعّل وضع الخادم المدمج من داخل الإعداد.

السلوك الحالي:
- إذا كانت القيمة `true`، يبدأ البوت لوحة الويب المدمجة دون الحاجة إلى `--server`
- إذا كانت `false` أو غير موجودة، فلا يزال بالإمكان تفعيل وضع الخادم من سطر الأوامر عبر `--server`

### `password`

Type: `string`, optional

كلمة المرور الخاصة بلوحة المتصفح.

هذا الحقل ثابت عند بدء التشغيل ويجب أن يكون سلسلة حرفية.

أولوية كلمة المرور الفعلية عند بدء التشغيل:
1. `BOT_PASSWD`
2. `--server-pass`
3. `server_config.password`
4. كلمة مرور يتم توليدها وطباعتها عند بدء التشغيل

### `port`

Type: `u16`, optional

منفذ الاستماع الخاص بالخادم المدمج.

السلوك الحالي:
- القيمة الافتراضية هي `9090`
- تقوم `--port` بتجاوز `server_config.port`

## `[auto_unwrap]`

قسم اختياري لحماية رصيد WSOL.

يكون هذا القسم ذا صلة عندما تكون قيمة `base_token` هي WSOL.

### `enabled`

Type: bindable `bool`

يحدد ما إذا كان يُسمح للبوت بفك WSOL تلقائيًا عندما ينخفض رصيد SOL في المحفظة عن الحد الأدنى المحدد.

### `minimum_balance`

Type: bindable `u64`, optional

الحد الأدنى لرصيد SOL في المحفظة، باللامبورت الخام.

السلوك الحالي:
- إذا كان هذا الحقل غير موجود، فلن يتم تطبيق أي منع للإرسال يعتمد على الرصيد
- إذا كان الحقل موجودًا وكانت `enabled = false`، فسيتم تخطي الإرسال العادي طالما أن رصيد SOL أقل من الحد
- إذا كان الحقل موجودًا وكانت `enabled = true`، فقد يرسل البوت معاملة صيانة لاستعادة SOL من WSOL

التنفيذ الحالي لـ auto-unwrap:
- مسار الصيانة مستقل عن إرسال التداول العادي
- يتم بناء معاملة versioned واحدة تقوم بإغلاق WSOL ATA القياسي ثم إعادة إنشائه داخل نفس المعاملة
- تستمر المحاولات حتى تُقبل المعاملة ويتم تأكيد تحقق النتيجة المطلوبة
- إذا لم يكن WSOL المتاح كافيًا لرفع رصيد SOL فوق الحد الأدنى المحدد، فسيظل الإرسال العادي متوقفًا ما دام الرصيد تحت الحد
...
### 7. يؤثر الإيقاف المؤقت على التداول، وليس على كل أعمال الصيانة

يوقف إيقاف البوت مؤقتًا إرسال التداول العادي، لكن مهام الصيانة المستقلة قد تستمر عند الحاجة.

### 8. لا يؤدي وضع السوق اليدوي إلى إيقاف تحديثات LUT

يقوم وضع السوق اليدوي بتعطيل إعادة تحميل `markets` التلقائية من المصدر المُعد، لكن إعادة تحميل LUT تستمر.
